...
SELECT COUNT(name) AS 'No Fee' FROM Facilities WHERE membercost = 0;
...
the member name. */
SELECT DISTINCT surname, firstname, Facilities.name FROM Bookings INNER JOIN Members ON Bookings.memid = Members.memid INNER JOIN Facilities ON Bookings.facid = Facilities.facid WHERE Facilities.name LIKE 'Tennis Court%' ORDER BY surname;
...
SELECT CONCAT(name, ' ', firstname, ' ', surname) AS sept_fourteen_bookings, CASE WHEN memid = 0 THEN slots * guestcost ELSE slots * membercost END AS cost FROM Bookings LEFT JOIN Facilities USING(facid) LEFT JOIN Members USING(memid) WHERE starttime LIKE '2012-09-14%' HAVING cost > 30 ORDER BY cost DESC;

/* Q9: This time, produce the same result as in Q8, but using a subquery. */
SELECT CONCAT(name, ' ', firstname, ' ', surname) AS sept_fourteen_bookings, CASE WHEN memid = 0 THEN slots * guestcost ELSE slots * membercost END AS cost FROM (SELECT * FROM Bookings WHERE starttime LIKE '2012-09-14%') AS sept_bookings LEFT JOIN Facilities USING(facid) LEFT JOIN Members USING(memid) WHERE (memid = 0 AND slots * guestcost > 30) OR (memid != 0 AND slots * membercost > 30) ORDER BY cost DESC;
